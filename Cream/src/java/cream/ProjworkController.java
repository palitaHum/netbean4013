...
public class ProjworkController implements Serializable {
    ...
    @EJB
    private cream.ProjworkFacade ejbFacade;
    ...
    public Projwork getSelected() {
        if (current == null) {
            ...
            current.setProjworkPK(new cream.ProjworkPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public Projwork getProjwork(cream.ProjworkPK id) {
    ...
    public static class ProjworkControllerConverter implements Converter {
        ...
        cream.ProjworkPK getKey(String value) {
            cream.ProjworkPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new cream.ProjworkPK();
            key.setProjno(values[0]);
            key.setEmpnum(values[1]);
            return key;
        }

        String getStringKey(cream.ProjworkPK value) {
